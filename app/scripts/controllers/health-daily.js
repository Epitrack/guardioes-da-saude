...
angular.module('gdsApp')
    .controller('HealthDailyCtrl', ['$scope', 'UserApi', '$translate', '$rootScope', 'LocalStorage', 'Notification', 'moment', function($scope, UserApi, $translate, $rootScope, LocalStorage, Notification, moment) {
        ...
        $translate(['00694']).then(function(translations) {
            singularSpelling = translations['00694'];
        });
        ...
        $translate(['00890']).then(function(translations) {
            $scope.totalstring = translations['00890'];
        });
        $scope.totalSpelling = $scope.goodSpelling = $scope.badSpelling = 'Participações';
        $translate(['00454']).then(function(translations) {
            $scope.totalSpelling = $scope.goodSpelling = $scope.badSpelling = translations['00454'];
        });
        ...
        $scope.getUserSurvey = function() {
            UserApi.getUserSurvey(function(data) {
                ...
                console.log("$scope.userSurvey", $scope.userSurvey);
        ...
        $scope.graphicLine = function() {
            ...
            if (days.length > 0) {
                $scope.lineOptions = {
                    data: days.reverse(),
                    xkey: 'y',
                    ykeys: [$scope.totalstring],
                    labels: [$scope.totalSpelling],
                    lineColors: ['#1E88E5'],
                    parseTime: false,
                    resize: true,
        ...
        $scope.graphicDonuts = function() {
            ...
            $scope.donutOptions = {
                data: [
                    ...
                    { label: $scope.malstring, value: $scope.userSurvey.symptom }
                ],
        ...
        $scope.vm.CalendarInterface = {
            ...
            getCalendarPopoverContent: function(q, day) {
                ...
                angular.forEach($scope.userCalendar, function(item) {
                    if (item.day === d) {
                        if (item.no_symptom) {
                            if (bem === 0) { bem = item.total; } else { bem += item.total; }
                        } else {
                            if (mal === 0) { mal = item.total; } else { mal += item.total; }
                        }
                    }
                });
                var content;
                if (q === 't') { content = (mal + bem); } else if (q === 'bem') { content = bem; } else { content = mal; }
                return content;
            },
